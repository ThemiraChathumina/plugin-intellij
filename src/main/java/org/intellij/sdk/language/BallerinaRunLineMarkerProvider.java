...
public class BallerinaRunLineMarkerProvider implements LineMarkerProvider {
    ...
    private boolean checkPublicFunction(@NotNull PsiElement element) {
        ...
        return parent.getFirstChild().getNextSibling().toString().equals("PsiElement(BallerinaTokenType.PUBLIC_KEYWORD)");
    }
    ...
    private boolean isMainFunction(@NotNull PsiElement element) {
        ...
        return element.getText().equals("main")
                && prevSibling != null
                && Objects.requireNonNull(prevSibling).toString().equals("PsiElement(BallerinaTokenType.FUNCTION_KEYWORD)")
                && element.getParent() != null
                && Objects.requireNonNull(element.getParent()).toString().equals("BallerinaFunctionDefnImpl(FUNCTION_DEFN)")
                && checkPublicFunction(element)
                && element.getParent().getParent() != null
                && Objects.requireNonNull(element.getParent().getParent()).toString().equals("BallerinaOtherDeclImpl(OTHER_DECL)");
    }
    ...
    private boolean isService(@NotNull PsiElement element) {
        return Objects.requireNonNull(element).toString().equals("PsiElement(BallerinaTokenType.SERVICE_KEYWORD)")
                && element.getParent() != null
                && Objects.requireNonNull(element.getParent()).toString().equals("BallerinaServiceDeclImpl(SERVICE_DECL)")
                && element.getParent().getParent() != null
                && Objects.requireNonNull(element.getParent().getParent()).toString().equals("BallerinaOtherDeclImpl(OTHER_DECL)");
    }
    ...
    private LineMarkerInfo createRunLineMarkerInfo(@NotNull PsiElement element) {
        ...
        return new LineMarkerInfo<>(
                ...
                () -> "Run " + finalFileName // Fallback tooltip text
        );
    }
