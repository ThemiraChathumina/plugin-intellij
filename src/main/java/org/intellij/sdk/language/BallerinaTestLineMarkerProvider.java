...
public class BallerinaTestLineMarkerProvider implements LineMarkerProvider {
    ...
    private boolean isTestFunction(@NotNull PsiElement element) {
        if (Objects.equals(element.getText(), "Config")
            ...
            && Objects.equals(element.getParent().getParent().getParent().toString(), "BallerinaMetadataImpl(METADATA)")
            && Objects.equals(element.getParent().getParent().getParent().getParent().toString(), "BallerinaFunctionDefnImpl(FUNCTION_DEFN)")
            && Objects.equals(element.getParent().getParent().getParent().getParent().getParent().toString(), "BallerinaOtherDeclImpl(OTHER_DECL)")) {
    ...
    private boolean isModuleTest(PsiElement element){
        ...
        if (virtualFile != null) {
            ...
            return parent.getName().equals("tests") && grandParent.getName().equalsIgnoreCase("modules") && balToml.exists();
        }
        return false;
    }
    ...
    private LineMarkerInfo createTestLineMarkerInfo(@NotNull PsiElement element) {
        ...
        return new LineMarkerInfo<>(
                ...
                AllIcons.RunConfigurations.TestState.Run, // Default run icon
                ...
                (e, elt) -> {
                    ...
                    if (file != null && file.getName().endsWith(".bal")) {
                        ...
                        String temp = configName.endsWith(".bal") ? configName.substring(0, configName.length() - 4) : configName;
                        RunnerAndConfigurationSettings settings = runManager.createConfiguration(temp, BallerinaTestConfigurationType.class);
                        BallerinaTestConfiguration testConfiguration = (BallerinaTestConfiguration) settings.getConfiguration();
...
//                        testConfiguration.addCommand("--tests");
                        if (isModuleTest(element))
                            testConfiguration.addCommand( "--tests " + packageName + "." + moduleName + ":" + getFunctionName(element));
                        else
                            testConfiguration.addCommand("--tests " + getFunctionName(element));


                        try {
                            ExecutionEnvironmentBuilder.create(project, DefaultRunExecutor.getRunExecutorInstance(), testConfiguration)
                                    .buildAndExecute();
                ...
                () -> "Test " + getFunctionName(element)  // Fallback tooltip text
        );
    }
