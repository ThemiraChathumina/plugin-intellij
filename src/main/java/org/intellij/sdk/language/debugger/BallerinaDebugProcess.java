...
import org.intellij.sdk.language.debugger.breakpoint.BallerinaBreakPointType;
import org.intellij.sdk.language.debugger.breakpoint.BallerinaBreakpointProperties;
    ...
    public void handleDebugHit(StoppedEventArguments args) {
        ApplicationManager.getApplication().invokeLater(() -> {
            ...
            try {
                ...
                if (Arrays.stream(threadsResp.getThreads()).noneMatch(t -> t.getId() == args.getThreadId().intValue())) {
                    return;
                }
                ...
                if (stackFrames.length > 0) {
                    ...
                    ((BallerinaSuspendContext) context).addToExecutionStack(Long.valueOf(args.getThreadId()), stackFrames);
                    XDebugSession session = getSession();
                    if (breakpoint == null) {
