...
public class BallerinaStackFrame extends XStackFrame {
    ...
    public XSourcePosition getSourcePosition() {
        ...
        return XDebuggerUtil.getInstance().createPosition(file, myFrame.getLine() - 1);
    }

    @Nullable
    private VirtualFile findFile() {
        String filePath = myFrame.getSource().getPath().replace("%20", " ");
        ...
        if (file != null && file.exists()) {
            System.out.println(LocalFileSystem.getInstance().findFileByPath(file.getAbsolutePath()).getPath());
    ...
    private File searchFile(File file, String search) {
        if (!file.isDirectory()) {
            ...
            String searchPath = searchFile.getPath();
            return filePath.endsWith(searchPath) || searchPath.endsWith(filePath) ? file : null;
        }
